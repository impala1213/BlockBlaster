...
public sealed class PieceDragView : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    private void AutoFillRefsIfMissing() // 인스펙터 미할당시 자동으로 보충
    {
    ...
    private void EnsureRaycastTargetOnRoot() // 드래그 시작 조건 보장용
    {
    ...
    private void TryAutoDetectHomeSlot() // 손패 Slot 탐지용 
    {
    ...
    public void OnDrag(PointerEventData eventData) // 드래그중 이동, 미리보기, 배치 가능한지 색상 표시
    {
        ...
        if (screenToGrid.TryGetGridPos(eventData.position, out var hover)) // 보드 위일시 셀 기반으로 이동 아니면 자유롭게 이동
        {
    ...
    public void OnEndDrag(PointerEventData eventData) // 드래그 종료시 처리 배치 or 슬롯으로 복귀
    {
    ...
    private bool IsReadyForDrag() // 드래그 가능한지 체크 조건들.
    {
        return piece != null &&
               piece.IsValid() &&
               blocks != null &&
    ...
    private void HandleGridRecalculated() // cellsize 계산
    {
        if (screenToGrid == null) return;
    ...
    private void ApplyTintFactor(Color factor) // 배치 가능시 초록 불가능시 빨강
    {
    ...
    private void EnsurePreviewBlocks() // block에 맞춰 미리보기 block 생성
    {
    ...
    private void UpdatePreview(Vector2Int origin, bool canPlace) // 이동시마다 미리보기 타일 표시
    {
        EnsurePreviewBlocks();
