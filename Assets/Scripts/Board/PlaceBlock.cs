...
public sealed class PlaceBlock
{
    public List<Vector2Int> GetOccupiedCells(PieceDefinition piece, Vector2Int origin)
    {
        var result = new List<Vector2Int>(piece.blocks.Length);
        for (int i = 0; i < piece.blocks.Length; i++)
            result.Add(origin + piece.blocks[i]);
